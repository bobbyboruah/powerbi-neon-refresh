    ...
    while running:
        try:
            ...
            with conn.cursor() as cur:
                ...
                if customers:
                    customer_tuples = [
                        (c["customer_id"], c["customer_name"], c["region"], c["signup_date"])
                        for c in customers
                    ]
                    execute_values(
                        cur,
                ...
                # Insert Inventory Next
                if inventory:
                    inventory_tuples = [
                        (i["sku"], i["product_name"], i["stock_qty"], i["reorder_level"], i["last_updated"])
                        for i in inventory
                    ]
                    execute_values(
                        cur,
                ...
                # Insert Sales Last (foreign key safe)
                sales = fetch_data("sales")
                if sales:
                    sales_tuples = [
                        (s["sale_id"], s["customer_id"], s["product_sku"], s["quantity"], s["sale_date"], s["revenue"])
                        for s in sales
                    ]
                    execute_values(
                        cur,
